...
class AppController():
    ...
    def _capture_view(self):
        self.camera._capture_camera_view()
    ...
    def _load_resolutions(self):
        selected_value = self.ids.drp_cameras.text
        cam_index = self.ids.drp_cameras.values.index(selected_value)
        ...
        if cam_index != 0:
            ...
            if platform == "android":
                ...
                for i in range(sizes.size()):
                    ...
                    if actual_size in base_resolutions:
                        if actual_size == recommended:
            ...
            self.ids.drp_resolution.text = 'Select an option'
        self.ids.drp_resolution.values = resolution_list
    ...
    def _start_validate_input(self):
    ...
    def _capture_validate_input(self):
        opt_camselected = 'network' if self.ids.opt_network.state == 'down' else 'camera' if self.ids.opt_camera.state == 'down' else None
        if opt_camselected is None:
            return False, 'Please select the device camera.'
        elif opt_camselected == 'network' and self.ids.txt_rtsp.text == '':
